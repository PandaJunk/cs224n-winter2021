...
def naiveSoftmaxLossAndGradient(
    ...
    p = y_head = softmax(numpy.dot(outsideVectors, centerWordVec))
    loss = -numpy.log(p[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + numpy.dot(numpy.transpose(outsideVectors), y_head)
    gradOutsideVecs = numpy.dot(y_head.reshape(-1, 1), centerWordVec.reshape(1, -1))
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - y_head[outsideWordIdx])
...
def negSamplingLossAndGradient(
    ...
    loss = -numpy.log(sigmoid(numpy.dot(numpy.transpose(outsideVectors[outsideWordIdx]), centerWordVec))) - \
           sum([numpy.log(sigmoid(numpy.dot(-numpy.transpose(outsideVectors[i]), centerWordVec))) for i in
                negSampleWordIndices])
    gradCenterVec = ((sigmoid(numpy.dot(numpy.transpose(outsideVectors[outsideWordIdx]), centerWordVec)) - 1) *
                     outsideVectors[outsideWordIdx]) + sum((1 - sigmoid(-numpy.dot(numpy.transpose(outsideVectors[i]),
                    centerWordVec))) * outsideVectors[i] for i in negSampleWordIndices)
    gradOutsideVecs = numpy.zeros(outsideVectors.shape, numpy.float64)
    for i in negSampleWordIndices:
        gradOutsideVecs[i] += ((1 - sigmoid(-numpy.dot(numpy.transpose(outsideVectors[i]), centerWordVec))) * centerWordVec)
    gradOutsideVecs[outsideWordIdx] = ((sigmoid(numpy.dot(numpy.transpose(outsideVectors[outsideWordIdx]), centerWordVec)) - 1) * centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordIdx in outsideWordsIdx:
        curr_loss, curr_gradCenterVec, curr_gradOutsideVectors = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += curr_loss
        gradCenterVecs[centerWordIdx] += curr_gradCenterVec
