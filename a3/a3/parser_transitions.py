...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        minibatch_sample = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch_sample)
        for i in range(len(transitions)):
            minibatch_sample[i].parse_step(transitions[i])
            if len(minibatch_sample[i].stack) == 1 and len(minibatch_sample[i].buffer) == 0:
                unfinished_parses.remove(minibatch_sample[i])
